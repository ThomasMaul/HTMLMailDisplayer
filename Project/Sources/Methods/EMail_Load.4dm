...
C_OBJECT:C1216($1;$form;$partindex;$attachment)  // contains Form
C_VARIANT:C1683($2)
C_TEXT:C284($part)
...
If (False:C215)
	$text:=JSON Stringify:C1217($form.mail)
...
If ($form.mail.attachments#Null:C1517)
	...
	If ($count>0)
		For each ($attachment;$form.mail.attachments)
			$cid:=String:C10($attachment.cid)
			C_BLOB:C604($content)
			$content:=$attachment.getContent()
			$filename:=Replace string:C233($attachment.name;":";"_")
			$filename:=Replace string:C233($filename;"/";"_")
			$filename:=Replace string:C233($filename;"\\";"_")
			BLOB TO DOCUMENT:C526($form.enclosurepath+$filename;$content)
			If ($cid#"")
				$old:="cid:"+$cid
				$new:=Convert path system to POSIX:C1106($form.enclosurepath+$filename)
...
$url:=$form.enclosurepath+"body.html"
